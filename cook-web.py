...
# with st.expander('5\) Results  \- Energy Demand & Cost'):
	...
	'Units':['hours/day','kWh/hour','kWh/day','kWh/year','kgCO2eq./kWh','kgCO2eq./year'],
	...
	bs_scc = (86 * usd_inr * (bs_tce/1000)).round(0) #social cost of carbon
	...
	el_scc = (86 * usd_inr * (el_tce/1000)).round(0) #social cost of carbon
...
with st.expander('5\) Results  \-Operating Specifics'):
	#dataframe - energy cost

	encons_var_v2  = {'Variable':['Daily cooking duration', 'Hourly consumption', 'Daily consumption', 'Annual consumption'],
	...
	df_encons_var_v2 = pd.DataFrame(encons_var_v2)
	df_encons_var_round = df_encons_var_v2.round(2)
	df_encons_var_round_v2 = df_encons_var_round.style.apply(highlight_greater, axis=1).format(precision=2)

	st.dataframe(df_encons_var_round_v2)	


with st.expander('6\) Results  \-Cost Specifics'):
	encost_var_test_v2  = {'Variable':['Unit cost', 'Opex', 'Overheads', 'Total operating cost'],
	'Units':['INR/kWh','INR/year','INR/year','INR/year'],
	'Baseline':[bs_uc,bs_opc.round(2),bs_ovc,bs_etc.round(2)],
	'e-Cooking':[el_uc_2,el_opc_2,el_ovc,el_etc_2],
	'Delta':[(el_uc_2 - bs_uc),(el_opc_2 - bs_opc).round(2),(el_ovc - bs_ovc).round(2),(el_etc_2 - bs_etc).round(2)]
	}
	df_encost_var_test_v2 = pd.DataFrame(encost_var_test_v2)
	# Apply the function to the DataFrame
	df_tcost_test_v2 = df_encost_var_test_v2.style.apply(highlight_greater, axis=1).format(precision=2)	
	st.dataframe(df_tcost_test_v2)

with st.expander('7\) Results  \- Social & Environmental Aspects'):
#dataframe - energy cost

	encons_var_v3  = {'Variable':['Unit carbon emission','Annual carbon emission'],
	'Units':['kgCO2eq./kWh','kgCO2eq./year'],
	'Baseline':	[bs_uce.round(2),bs_tce.round(2)],
	'e-Cooking':[el_uce.round(2),el_tce.round(2)],
	'Delta':[(el_uce- bs_uce).round(2),(el_tce - bs_tce).round(2)]
	}
	df_encons_var_v3 = pd.DataFrame(encons_var_v3)
	df_encons_var_round_v3 = df_encons_var_v3.round(2)

	encost_var_v3  = {'Variable':['Social Carbon Cost'],
	'Units':['INR/year'],
	'Baseline':[bs_scc],
	'e-Cooking':[el_scc],
	'Delta':[(el_scc - bs_scc)]
...
with st.expander('9\) Results  \- Financing'):
	#baseline - financing
...
st.markdown('<mark>*Values mentioned are based on research papers and empirical evidence.*</mark>', unsafe_allow_html=True)

st.markdown('<mark>*This analysis is a part of Deep Electrification initiative by Vasudha Foundation' +
	     ' with support from SED Fund. For any queries, collaboration or further use of this research.' + 
		 ' Please drop a mail to bikash@vasudhaindia.org*</mark>', unsafe_allow_html=True)
