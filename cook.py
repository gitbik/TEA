...
# with st.expander('6\) Results  \- Energy Cost'):
	...
	#el_price3 = el_price2.loc[el_price2['State'] == state_select,el_price2['Source'] == epf_select,soc_eco_select] #unit cost
	...
	encost_var  = {'Variable':['Unit cost', 'Opex', 'Overheads', 'Total cost','Social Carbon Cost'],
	'Units':['INR/kWh','INR/year','INR/year','INR/year','INR/year'],
	'Baseline':[bs_uc,bs_opc.round(2),bs_ovc,bs_etc.round(2),bs_scc],
	'e-Cooking':[el_uc,el_opc.round(2),el_ovc,el_etc.round(2),el_scc],
	'Delta':[(el_uc - bs_uc).round(2),(el_opc - bs_opc).round(2),(el_ovc - bs_ovc).round(2),(el_etc - bs_etc).round(2), (el_scc - bs_scc)]
	}
...
with st.expander('6\) Results  \- Health Impacts'):
	...
	hlt_var  = {'Variable':['Daily IHAP (PM 2.5)','Annual IHAP (PM 2.5)'],
	'Units':['μg/m3','mg/m3'],
	#'Baseline':[bs_dihap.round(2),bs_ahap.round(2),bs_haz,bs_mort],
	'Baseline':[bs_dihap.round(2),bs_ahap.round(2)],
	#'e-Cooking':[el_dihap.round(2),el_ahap.round(2),el_haz,el_mort],
	'e-Cooking':[el_dihap.round(2),el_ahap.round(2)],
...
with st.expander('7\) Results  \- Financing'):
	...
	el_sav = [-(el_tcost-bs_tcost)]+[-((el_etc - bs_etc))] 
	#el_uce = df_el_cf4['Unit carbon emission'].values[0]
	...
	fin_var  = {'Variable':['Annual Income of HH','Payback period','Annual Savings'],
	'Units':['INR','years','INR'],
	'Baseline':[bs_aninc,'-','-'],
	'e-Cooking':['NA',el_pbp.round(2),el_sav.round(2)],
	#'Delta':['-','-','-']
	}
